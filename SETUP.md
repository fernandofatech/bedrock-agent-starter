...
Connect this repository to Vercel and expose it on `bedrock-agent.moretes.com` via
Cloudflare DNS.

## 1. Create the Vercel project

1. Sign in at https://vercel.com with the GitHub account that owns this repo.
2. Click **Add New… → Project** and import `fernandofatech/bedrock-agent-starter`.
...
```bash
gh secret set VERCEL_TOKEN      --body "<token>" -R fernandofatech/bedrock-agent-starter
gh secret set VERCEL_ORG_ID     --body "<org-id>" -R fernandofatech/bedrock-agent-starter
gh secret set VERCEL_PROJECT_ID --body "<project-id>" -R fernandofatech/bedrock-agent-starter
```

## 4. Attach the custom subdomain

In Vercel: **Settings → Domains → Add** → `bedrock-agent.moretes.com`.
...
```bash
curl -I https://bedrock-agent.moretes.com
```
